...
function isDepthMaterial(target: any): target is THREE.MeshDepthMaterial {
	return (target && target.isMeshDepthMaterial) || target === THREE.MeshDepthMaterial;
}
...
function useUniformPairs(instance, uniforms) {
	...
	if (instance.lightMap) cloneUniforms(UniformsLibExtendable.lightmap, uniforms);

	if (instance.bumpMap) cloneUniforms(UniformsLibExtendable.bumpmap, uniforms);

	if (instance.normalMap) cloneUniforms(UniformsLibExtendable.normalmap, uniforms);

	if (instance.displacementMap) cloneUniforms(UniformsLibExtendable.displacementmap, uniforms);
...
function useUniforms(name, object, uniforms) {
	...
	if (object.common !== false) cloneUniforms(UniformsLibExtendable.common, uniforms);

	if (requiredUniforms[name] !== undefined) cloneUniforms(requiredUniforms[name], uniforms);

	const fog =
		object.fog || (object.material ? object.material.fog || object.material.useFog : null);
