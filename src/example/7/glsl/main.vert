...
void main() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        vec2 noiseFreq = vec2(uFreqX, uFreqY);

        float noiseCeil = 0.6 + float(i) * 0.07;

        float noise = smoothstep(uNoiseFloor, noiseCeil * uNoiseCeil,
        snoise(vec3(
            noiseCoord.x * noiseFreq.x + uTime * noiseFlow,
            noiseCoord.y * noiseFreq.y,
            uTime * noiseSeed + noiseSeed)
            )
        );
        vColor = mix(vColor, uColor[i], noise);
    }

    vec3 pos = vec3(position.x, position.y, position.z + noise * .1 + tilt + offset);
